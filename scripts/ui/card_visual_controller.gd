...
const CARD_HEIGHT: float = 98.0
const HOLE_LEFT: int = 11
const HOLE_TOP: int = 16
const HOLE_WIDTH: int = 52
const HOLE_HEIGHT: int = 32
...
# -- Frame loading (PNG assets, procedural fallback) --

static func _frame_path(type: int) -> String:
	match type:
		0: return FRAMES_DIR + "frame_marble.png"
		1: return FRAMES_DIR + "frame_power_up.png"
		2, 4: return FRAMES_DIR + "frame_trick.png"
		3: return FRAMES_DIR + "frame_terrain.png"
	return FRAMES_DIR + "frame_marble.png"


static func _get_frame(type: int) -> Texture2D:
	if type in _frame_cache:
		return _frame_cache[type]
	var path := _frame_path(type)
...
static func _frame_fill(type: int) -> Color:
	match type:
		0: return Color(0.18, 0.10, 0.08, 1.0)
		1: return Color(0.08, 0.12, 0.22, 1.0)
		2: return Color(0.08, 0.20, 0.10, 1.0)
		3: return Color(0.20, 0.16, 0.10, 1.0)
		4: return Color(0.18, 0.10, 0.22, 1.0)
	return Color(0.1, 0.1, 0.1, 1.0)


static func _frame_accent(type: int) -> Color:
	match type:
		0: return Color(0.9, 0.45, 0.25, 1.0)
		1: return Color(0.25, 0.50, 0.9, 1.0)
		2: return Color(0.25, 0.80, 0.35, 1.0)
		3: return Color(0.65, 0.50, 0.30, 1.0)
		4: return Color(0.65, 0.35, 0.80, 1.0)
	return Color.WHITE


# -- Sprite loading (PNG assets, procedural fallback) --
...
static func _get_card_sprite(card_data: CardData) -> Texture2D:
